...
def _has_distribution(dist_name: str) -> bool:
    try:
        get_distribution_version(dist_name)
        return True
...
def _is_gfx906_triton_distribution() -> bool:
    # `nlzy/triton-gfx906` keeps the import module as `triton` while changing
    # distribution/package naming. Probe common dist names for compatibility.
    return _has_distribution("triton-gfx906") or _has_distribution("triton_gfx906")


def _skip_active_driver_check() -> bool:
    # Opt-in escape hatch for Triton variants that do not expose
    # `triton.backends` with upstream semantics.
    return os.getenv("VLLM_TRITON_SKIP_ACTIVE_DRIVER_CHECK", "0") == "1"


HAS_TRITON = _has_module("triton") or _has_module("pytorch-triton-xpu")
TRITON_IS_GFX906_VARIANT = False
if HAS_TRITON:
    TRITON_IS_GFX906_VARIANT = _is_gfx906_triton_distribution()
    skip_active_driver_check = (
        _skip_active_driver_check() or TRITON_IS_GFX906_VARIANT
    )

    if skip_active_driver_check:
        logger.info_once(
            "Skipping Triton active-driver probe for compatibility mode."
        )
    else:
        try:
